 ...
 */
        ...
        timeCell.appendChild(timeText);
          ...
          for (let element of event.data.orders) {         
            if (element.source == null || element.source  == event.data.plysrc ) {
              ...
              let amountCell = row.insertCell();
              let amountText = document.createTextNode("x"+element.amount);
              amountCell.appendChild(amountText);
              ...
              locationCell.innerHTML = '<button class="button-loc"   role="button" onClick=\'DoAction(' + element.orderId + ',"showLocation")\' >查看地址</button>';

              if (element.source == null) {
                let takeCell = row.insertCell();
                takeCell.innerHTML = '<button class="button-take"   role="button" onClick=\'DoAction(' + element.orderId + ',"takeOrder")\' >接单</button>';
              } else {
                let cancelCell = row.insertCell();
                cancelCell.innerHTML = '<button class="button-cancel"  role="button" onClick=\'DoAction(' + element.orderId + ',"cancelOrder")\' >取消订单</button>';
              }
    ...
    $('#switch').click(function (event) {
        $.post('https://Sam_delivery/action', JSON.stringify({
          event :"orderNotify",
          value : this.checked
      }));
    });
