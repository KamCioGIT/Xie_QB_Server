    ...
    if (type == 'Wheat') {
        ...
        for (var k in data){
            ...
            <div class="middlebar_bar">
                ...
                <img src="./img/${data[Number(k)].ItemImage}.png">
                <span id="moneytext">$${data[Number(k)].Price}</span>
                <div class="sellbuton" data-item="${data[Number(k)].ItemCode}" data-price="${data[Number(k)].Price}">出售</div>
            </div>
            `
    ...
    window.addEventListener("message", function(event) {
        ...
        switch (event.data.action) {
            case "open":
                ...
                break;
            case "close":
                $.post("https://Sam-farming/ResetClothes");
                $(".shopingbarlist").html("");
                SelectedItems = [];
                Amounts = [];
                AllItems = [];
...
$(document).on('click', '.sellbuton', function(e) {
    ...
    if (currshop == 'buy') {
        ...
        $.post("https://Sam-farming/SellItem", JSON.stringify({
            price: price,
            item: itemCode
        }));
    }
...
$(document).on('click', '.farmingsell', function(e) {
    e.preventDefault()
    if (currshop != 'sell') {
        $.post("https://Sam-farming/GetSellConfig", function(data) {
...
$(document).on('click', '.farmingbuy', function(e) {
    ...
    if (currshop != 'buy') {
        currshop = "buy";
        $.post("https://Sam-farming/GetShopConfig", function(data) {
...
$(document).on('click', ".kutucukbar", function(e) {
    e.preventDefault()
    let id = $(this).attr("id")
    $.post("https://Sam-farming/StartJob", JSON.stringify({
        id: id
    }), function(cb) {
        if (cb) {
            FadeOut(".budaykontaynir", 350)
...
$(document).on('click', ".clotinghbir", function(e) {
    e.preventDefault()
    $.post("https://Sam-farming/WearClothes");
});

$(document).on('click', '.cashdiv', function(e) {
    e.preventDefault()
    $.post("https://Sam-farming/buy", JSON.stringify({
