...
plyRequests, deadPlayers, DeathInjury, Outfits, PlayerInjury, HospitalBeds = {}, {}, {}, {}, {}, {}
...
AddEventHandler('playerDropped', function ()
    ...
    if DeathInjury[playerId] then DeathInjury[playerId] = nil end
    if PlayerInjury[playerId] then PlayerInjury[playerId] = nil end
    if plyRequests[playerId] then
        plyRequests[playerId] = nil
        TriggerClientEvent('wasabi_ambulance:syncRequests', -1, plyRequests, true)
    end
    if not next(HospitalBeds) then return end
    local found
    for _,v in pairs(HospitalBeds) do
        if v.inUse and v.inUse == playerId then
            v.inUse = false
            found = true
            break
        end
    end
    if found then GlobalState.HospitalBeds = HospitalBeds end
end)
...
RegisterNetEvent('wasabi_ambulance:setDeathStatus', function(isDead, sql)
    local src = source
...
RegisterNetEvent('wasabi_ambulance:injurySync', function(injury)
    local src = source
    if not injury then PlayerInjury[src] = nil DeathInjury[src] = nil return end
    if type(injury) == 'table' then PlayerInjury[src] = injury return end
    ...
    if wsb.hasItem(src, Config.EMSItems.revive.item) > 0 then
        ...
        if Config.ReviveRewards.enabled then
            ...
            if reward > 0 then
                ...
                TriggerClientEvent('wasabi_bridge:notify', src, Strings.player_successful_revive, (Strings.player_successful_revive_reward_desc):format(reward), 'success')
            else
                TriggerClientEvent('wasabi_bridge:notify', src, Strings.player_successful_revive, Strings.player_successful_revive_desc, 'success')
            end
        else
            TriggerClientEvent('wasabi_bridge:notify', src, Strings.player_successful_revive, Strings.player_successful_revive_desc, 'success')
        end
...
RegisterNetEvent('wasabi_ambulance:healPlayer', function(targetId)
    ...
    if targetId == src then
        if wsb.hasItem(src, Config.EMSItems.heal.item) > 0 then
            ...
            TriggerClientEvent('wasabi_ambulance:heal', src, false, true)
            TriggerClientEvent('wasabi_bridge:notify', src, Strings.used_meditkit, Strings.used_medikit_desc, 'success')
        end
    elseif wsb.hasGroup(src, Config.ambulanceJob) or authorized then
        if wsb.hasItem(src, Config.EMSItems.heal.item) > 0 then
            if Config.EMSItems.heal.remove then
                wsb.removeItem(src, Config.EMSItems.heal.item, 1)
            end
            TriggerClientEvent('wasabi_bridge:notify', src, Strings.player_successful_heal, Strings.player_successful_heal_desc, 'success')
...
RegisterNetEvent('wasabi_ambulance:treatPlayer', function(target, injury)
    ...
    if target > 0 then
        ...
        if wsb.hasGroup(src, Config.ambulanceJob) or authorized then
            if wsb.hasItem(src, Config.TreatmentItems[injury]) > 0 then
                ...
                TriggerClientEvent('wasabi_bridge:notify', src, Strings.player_treated, Strings.player_treated_desc, 'success')
            end
        end
    end
end)
    ...
    if target > 0 then
        ...
        if wsb.hasGroup(src, Config.ambulanceJob) or authorized then
            if wsb.hasItem(src, Config.EMSItems.sedate.item) > 0 then
                ...
                TriggerClientEvent('wasabi_bridge:notify', target, Strings.target_sedated, Strings.target_sedated_desc, 'inform')
                TriggerClientEvent('wasabi_bridge:notify', src, Strings.target_sedated, Strings.player_successful_sedate_desc, 'success')
...
RegisterNetEvent('wasabi_ambulance:placeOnStretcher', function(target)
...
RegisterNetEvent('wasabi_ambulance:restock', function(data)
    local src = source
    if wsb.hasGroup(src, Config.ambulanceJob) then
...
lib.callback.register('wasabi_ambulance:getPlayerNames', function(source)
    local src = source
    if not wsb.hasGroup(src, Config.ambulanceJob) then return end
    local players = GetPlayers()
    local data = {}
    ...
    for k, playerId in pairs(players) do
        ...
        if not wsb.hasGroup(playerId, Config.ambulanceJob) then goto continue end
...
lib.callback.register('wasabi_ambulance:getPlayerCoords', function(source)
    ...
    if not wsb.hasGroup(source, Config.ambulanceJob) then return end
    local players = GetPlayers()
    local data = {}
...
RegisterNetEvent('wasabi_ambulance:billPlayer', function(target, amount)
    ...
    if player then
        ...
        exports.pefcl:createInvoice(src, { to = player.getName(), toIdentifier = identifier, from = 'Hospital', fromIdentifier = nil, amount = amount, message = 'Medical Bills', receiverAccountIdentifier = Config.ambulanceJob, expiresAt = nil })
    end
end)
...
RegisterNetEvent('wasabi_ambulance:qbBill', function(target, amount, job)
    ...
    TriggerClientEvent('wasabi_bridge:notify', src, Strings.fine_sent, (Strings.fine_sent_desc):format(addCommas(amount)), 'success')
    TriggerClientEvent('wasabi_bridge:notify', target, Strings.fine_received, (Strings.fine_received_desc):format(addCommas(amount)), 'error')
    ...
    wsb.registerUsableItem(Config.GPSBlips.item, function(source)
        ...
        for k, playerId in pairs(players) do
            ...
            if ACTIVEGPS[source] then
                TriggerClientEvent('wasabi_bridge:notify', playerId, Strings.gps_active, (Strings.gps_active_desc):format(wsb.getName(source)), 'info')
            else
                TriggerClientEvent('wasabi_bridge:notify', playerId, Strings.gps_deactive, (Strings.gps_deactive_desc):format(wsb.getName(source)), 'info')
            end
...
RegisterNetEvent('wasabi_ambulance:saveOutfit', function(data)
    if not source or source == 0 then return end
    local src = source
    local identifier = wsb.getIdentifier(src)
    if not identifier then return end
    if Outfits[identifier] then return end
    Outfits[identifier] = data
end)
...
RegisterNetEvent('wasabi_ambulance:setPlayerState', function(id, data, target)
    local target = target or nil
    if not target then target = source end
    local state = Player(target).state
    state:set(id, data, true)
end)

lib.callback.register('wasabi_ambulance:loadPreviousOutfit', function(source)
    local src = source
    local identifier = wsb.getIdentifier(src)
    if not identifier then return false end
    if not Outfits[identifier] then return false end
    return Outfits[identifier]
end)
