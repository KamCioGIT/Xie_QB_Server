...
$(document).ready(function () {
  window.addEventListener("message", function (event) {
    if (event.data.action == "OpenPaintBall") {
      ...
      $("#red-total").html("总玩家数 "+ event.data.red)
      $("#blue-total").html("总玩家数 "+event.data.blue)
      ...
      $(".total-lobby").html("总房间数 " + event.data.TotalLobbys);
      ...
      <div class="lobbys-l-join" id="` +
          event.data.name +
          `" onclick="LoginLobby(this.id)">加入</div>
    </div>
    `;
...
function openCreateLobby() {
  tyx = "create";
  $(".create-lobby-dec").html("如果您想没有密码，请留空.")
  $(".create-button").html("创建大厅")
  $.post("https://BakiTelli_paintball/addname");
}
function LoginLobby(name) {
  tyx = "login";
  $(".create-lobby-dec").html("请注意，密码仅由数字组成..")
  $(".create-button").html("登录大厅")
  $(".Create-Password").val("");
  $.post(
...
function DefaultPlayers() {
  ...
  teams.forEach(function (team) {
    players.forEach(function (player) {
      ...
      $(selector + " .team-name").html("加入团队");
      $(selector + " .team-id").html("无");
    });
  });
}
