...
function closeQuestion(passed) {
	$.post('https://Sam_boatschool/action', JSON.stringify({
		action: 'close',
		passed: passed
	}));
}
...
$(".theory-next").click(function(e) {
	...
	if ($('input[name="question"]:checked').val()) {
	  	if(questionNumber != QuestionToAnswer){
			...
			questionNumber++;
			openQuestion()
			$.post('https://Sam_boatschool/action', JSON.stringify({action: 'playSound', type: 'nextQuestion'}));
		} else {
			answers.push($('input[name="question"]:checked').val());
			var nbGoodAnswer = 0;
			...
			if (nbGoodAnswer >= NeedAnswersToPass) {
				...
				$.post('https://Sam_boatschool/action', JSON.stringify({action: 'playSound', type: 'passedExam'}));
			} else {
				...
				$.post('https://Sam_boatschool/action', JSON.stringify({action: 'playSound', type: 'failedExam'}));
			}
		}
		return false;
	}
