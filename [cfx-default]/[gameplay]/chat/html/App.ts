    ...
    methods: {
        ...
        ON_MORELINE({ status }: { status: boolean }) {
            ...
            if (chatWindows.length > 0) {
                const chatWindow = chatWindows[0] as HTMLElement;
                const chatInput = chatInputs[0] as HTMLElement;
                if (status) {
                    chatWindow.style.minHeight = '40%';
                    chatInput.style.top = '36.8%';
                } else {
                    chatWindow.style.minHeight = '25%';
                    chatInput.style.top = '28.8%';
                }
            }
        },
