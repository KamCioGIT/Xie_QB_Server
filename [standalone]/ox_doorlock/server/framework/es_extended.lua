...
function IsPlayerInGroup(player, filter)
    ...
    if type == 'string' then
        ...
        elseif player.metadata['gang'] == filter then
            return player.metadata['gang'], 5
        end
    else
        ...
        if tabletype == 'hash' then
            ...
            grade = filter[player.metadata['gang']]
            if grade and tonumber(grade) <= 5 then
                return player.metadata['gang'], 5
            end
        elseif tabletype == 'array' then
            for i = 1, #filter do
                if player.job.name == filter[i] then
                    return player.job.name, player.job.grade
                elseif player.metadata['gang'] == filter[i] then
                    return player.metadata['gang'], 5
                end
            end
        end
