...
RegisterNetEvent('sam_doorlock:changeDoorGroup', function(group, group2)
	for k, door in pairs(doors) do
		for a, b in pairs(door.groups) do
			if a == group then
				door.groups[group2] = b
				door.groups[a] = nil
				MySQL.update('UPDATE ox_doorlock SET data = ? WHERE id = ?', { encodeData(door), k })
				TriggerClientEvent('ox_doorlock:editDoorlock', -1, k, door)
			end
		end
	end
end)
