...
const ItemsModal: React.FC<Props> = ({ modal, setModal }) => {
  ...
  return (
    <form onSubmit={form.onSubmit((values) => handleSubmit(values))}>
      <Stack>
        <TextInput label="Metadata數據" defaultValue={itemData.metadata} {...form.getInputProps('metadata')} />
        <Switch label="使用後刪除" defaultChecked={itemData.remove} {...form.getInputProps('remove')} />
